  ...
  <q-layout view="lHr Lpr fFf">
    ...
    <q-drawer v-model="rightDrawerOpen" show-if-above bordered class="right-drawer" side="right">
      <q-list>
        ...
        <q-item>
          <q-item-section>
            <q-btn @click="updatePWA">Actualizar</q-btn>
          </q-item-section>
        </q-item>
        <q-item>
          <q-item-section>
            <q-btn @click="uninstallPWA">Reiniciar App</q-btn>
          </q-item-section>
        </q-item>
...
function postMsg(sw, sAction, pResp) {
  try {
    ...
    this.log += sAction + ' error: ' + err;
  }
}

function fetchVersionFromSW() {
  if (navigator.serviceWorker.controller) {
    postMsg(navigator.serviceWorker.controller, 'getVersion', (event) => {
      if (event.data && event.data.version) {
