  ...
  <q-dialog v-model="isOpen" persistent class="q-pa-md centered-modal">
    <q-card class="modal-card">
      <q-toolbar class="navbar">
        <q-toolbar-title>Editar Factura</q-toolbar-title>
...
const isOpen = computed(() => modalStore.facturaEditIsOpen);
...
const iva = computed(() => subtotal.value * (factura.value.ivaPorcentaje / 100));
...
const $q = useQuasar();
const emit = defineEmits(['factura-editada']);
...
const guardarFactura = () => {
  ...
  const indice = facturas.findIndex(nota => nota.numero === factura.value.numero);
  if (indice !== -1) {
    facturas[indice] = { ...factura.value };
  } else {
    facturas.push({ ...factura.value });
  }
  $q.localStorage.set('facturas', facturas);
  emit('factura-editada', factura.value);
  closeModal();
};

watch(isOpen, (val) => {
  if (val && modalStore.facturaToEdit) {
    factura.value = { ...modalStore.facturaToEdit };
  }
});

watch(
  () => factura.value.productosFactura,
  () => {
    subtotal.value; // Trigger subtotal recalculation
  },
  { deep: true }
);

watch(
  () => factura.value.ivaPorcentaje,
  () => {
    total.value; // Trigger total recalculation
  }
);
</script>
